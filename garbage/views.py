...
config = {
	'apiKey': "AIzaSyCcoq_hK1ZZVq0w88VTXnU9Hwpxgh768fg",
       'authDomain': "http://possible-point-343219.firebaseapp.com/",
       'databaseURL': "https://inter-iit-60162-default-rtdb.firebaseio.com/",
       'projectId': "possible-point-343219",
       'storageBucket': "http://possible-point-343219.appspot.com/",
       'messagingSenderId': "254674698161",
       'appId': "1:254674698161:web:0e6522efb805c097824790",
       'measurementId': "G-KQ0BEGCF2Y"
}
firebase = pyrebase.initialize_app(config)
authe = firebase.auth()
database=firebase.database()
print(database.child("Bin").get())

def signIn(request):
    
    print(database.child("Bin").get().val())
...
def get_latlong(request) :
    ...
    for i in bin:
        ...
        aqi= (float(db.child("Bin").child(i).child("aqi").get().val()))
        weight= (float(db.child("Bin").child(i).child("weight").get().val()))
        print(i)
        try :
...
def get_latlong2(request) :
   ...
   bin = database.child("Bin").get().val()
   bin2 = database.child("BinPerLevel").get().val()
   lat, lon, cap = [], [], []
   cap_70, cap_20, cap_20_70 = [], [], []
   for i in bin:
       height = (int(database.child("Bin").child(i).child("height").get().val()))
       lati = (float(database.child("Bin").child(i).child("latitude").get().val()))
       long = (float(database.child("Bin").child(i).child("longitude").get().val()))
       print(i,height)
       try:
           print(date.today())
           #data = database.child("BinPerLevel").child(i).child(str(date.today())).get().val()
           #print(data)
           #last = next(reversed(data))
...
def generate_routes(request):
    ...
    datap['vehicle_capacities'], datap['vehicle_key'] = [150,150,150,150],[1,2,3,4]
